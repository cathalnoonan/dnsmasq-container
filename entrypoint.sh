...
# Define base paths
TEMPLATE_ROOT="${TEMPLATE_ROOT:-/opt/dnsmasq/templates}"
DESTINATION_ROOT="${DESTINATION_ROOT:-/opt/dnsmasq}"

replace_placeholders() {
    local template_file="${1}"

    # 1. Validation: Ensure template is actually within the TEMPLATE_ROOT
    # This prevents directory traversal via ../
    case "$(realpath "$template_file")" in
        "$(realpath "$TEMPLATE_ROOT")"*) ;; # Path is valid, proceed
    ...
    local relative_path="${template_file#$TEMPLATE_ROOT/}"
...
if [ -d "$TEMPLATE_ROOT" ]; then
    ...
    find "$TEMPLATE_ROOT" -type f -name "*.conf" | while read -r file; do
        replace_placeholders "$file"
    done
else
    echo "[Setup Info] Template directory '$TEMPLATE_ROOT' not found, skipping template processing."
fi

echo "[Setup Info] Starting dnsmasq..."
echo
exec dnsmasq -k --conf-file="${DESTINATION_ROOT%/}/dnsmasq.conf"
